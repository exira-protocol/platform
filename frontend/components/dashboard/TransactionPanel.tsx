...
export const TransactionPanel: React.FC<TransactionPanelProps> = ({
  ...
  <div className="space-y-4">
    ...
    <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm">
          1 {selectedToken.symbol} ≈ ₹{selectedToken.price.toFixed(2)} USDC
        </span>
      </div>
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
    ...
    <Button className="w-full bg-black text-white hover:bg-gray-800 text-base py-5" size="lg">
      CONNECT WALLET
    </Button>

    <div className="space-y-2 pt-2">
      <div className="flex justify-between text-sm">
        <span className="text-gray-600 dark:text-gray-400 underline">Allowance</span>
        <span>{selectedToken.maxAllowance.toLocaleString()} {selectedToken.symbol}</span>
      </div>
      <div className="flex justify-between text-sm">
      ...
      <div className="flex justify-between text-sm">
        <span className="text-gray-600 dark:text-gray-400">Instantly Available</span>
        <span>{selectedToken.instantlyAvailable.toLocaleString()} {selectedToken.symbol}</span>
      </div>
    </div>
  </div>
)
